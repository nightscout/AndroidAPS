...
class ActionUAMSMBMaxMinutesChangeTest : ActionsTestBase() {
    ...
    @BeforeEach fun setUp() {
        whenever(rh.gs(R.string.changeUamSmbMaxMinutesTo)).thenReturn("Change UAM SMB max minutes to %1\$d min")

        sut = ActionUAMSMBMaxMinutesChange(injector)
    }

    @Test fun friendlyName() = runTest {
        assertThat(sut.friendlyName()).isEqualTo(R.string.changeUamSmbMaxMinutes)
    }
    ...
    @Test fun fromJSONTest() = runTest {
        sut.fromJSON("""{"minutes":60}""")
        assertThat(sut.minutes.value).isEqualTo(60)
    }
}
