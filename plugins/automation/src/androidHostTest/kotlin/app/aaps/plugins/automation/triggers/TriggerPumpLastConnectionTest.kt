...
//        and so is every last connection time
        ...
        var t = TriggerPumpLastConnection(triggerDeps).setValue(110).comparator(Comparator.Compare.IS_EQUAL)
        assertThat(t.minutesAgo.value).isEqualTo(110)
        assertThat(t.comparator.value).isEqualTo(Comparator.Compare.IS_EQUAL)
        assertThat(t.shouldRun()).isFalse()
        t = TriggerPumpLastConnection(triggerDeps).setValue(10).comparator(Comparator.Compare.IS_EQUAL)
        assertThat(t.minutesAgo.value).isEqualTo(10)
        assertThat(t.shouldRun()).isFalse() // 0 == 10 -> FALSE
        t = TriggerPumpLastConnection(triggerDeps).setValue(5).comparator(Comparator.Compare.IS_EQUAL_OR_GREATER)
        assertThat(t.shouldRun()).isTrue() // 5 => 0 -> TRUE
        t = TriggerPumpLastConnection(triggerDeps).setValue(310).comparator(Comparator.Compare.IS_EQUAL_OR_LESSER)
        assertThat(t.shouldRun()).isFalse() // 310 <= 0 -> FALSE
        t = TriggerPumpLastConnection(triggerDeps).setValue(420).comparator(Comparator.Compare.IS_EQUAL)
        assertThat(t.shouldRun()).isFalse() // 420 == 0 -> FALSE
    }

    @Test fun copyConstructorTest() = runTest {
        val t: TriggerPumpLastConnection = TriggerPumpLastConnection(triggerDeps).setValue(213).comparator(Comparator.Compare.IS_EQUAL_OR_LESSER)
    ...
    @Test fun toJSONTest() = runTest {
        val t: TriggerPumpLastConnection = TriggerPumpLastConnection(triggerDeps).setValue(410).comparator(Comparator.Compare.IS_EQUAL)
    ...
    fun fromJSONTest() = runTest {
        val t: TriggerPumpLastConnection = TriggerPumpLastConnection(triggerDeps).setValue(410).comparator(Comparator.Compare.IS_EQUAL)
        val t2 = triggerFactory.instantiate(t.toJSON().asJsonObject()) as TriggerPumpLastConnection
