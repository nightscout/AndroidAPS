 ...
 *        X < -0.44 -> SingleDown
 *    2.2 If it cannot calculate (previous sgvId not found OR previous value missing), fallback Flat.
 */
object InstaraTrendArrowResolver {
     ...
     *
     * @param instaraDirectionRaw direction string from Instara JSON (maybe missing/invalid/NONE).
    ...
    suspend fun resolve(
        ...
        val prevValueMgdl: Double? = try {
            persistenceLayer.getGlucoseValueByPumpIdAndSource(SourceSensor.INSTARA, prevId)?.value
        } catch (_: Throwable) {
            null
        }
