...
class IobCobCalculatorPlugin(
    ...
    override suspend fun onStart() {
        ...
        // Resilient collectors: an uncaught exception inside a plain onEach cancels the flow
        // collection forever and the plugin would stop reacting to DB changes (issue #5066).
        ...
        persistenceLayer.observeChanges(CA::class)
            .collectResilient(newScope, aapsLogger, LTag.AUTOSENS) { list -> list.minOfOrNull { it.timestamp }?.let { scheduleHistoryDataChange(it, reloadBgData = false) } }
    ...
    override suspend fun forceRecalculation(reason: String) {
        // No stopCalculation and no cache invalidation needed: history did not change.
        // Starting a job replaces the run already holding that name, but the replaced run
        // keeps going until its next isStopped check. PrepareGraphDataRunner.publishAds()
        // makes sure such a replaced run does not publish its stale result over the
        // result of this forced chain.
        calculationWorkflow.runCalculation(
            job = CalculationWorkflow.MAIN_CALCULATION,
            iobCobCalculator = this,
            overviewData = overviewData,
                ...
                withContext(NonCancellable) {
                    historyLock.withLock {
                        try {
                            ...
                            // Without this catch the error would only reach the scope and the
