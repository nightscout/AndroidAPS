...
class TbrGateTest : TestBase() {
    ...
    fun `disabled loop allows all command kinds`() {
        // Loop is disabled but pump is still operated manually by the user.
        TbrGate.CommandKind.entries.forEach { kind ->
    ...
    fun `resume mode allows all command kinds`() {
        // RESUME is a transient signal, not a stable active mode, but defensively allow.
        TbrGate.CommandKind.entries.forEach { kind ->
    ...
    fun `super bolus allows bolus`() {
        // The super bolus itself is a regular bolus command.
    ...
    fun `every mode and command kind combination produces a Decision`() {
        // Regression guard: whenever a new RM mode or CommandKind is added, this test forces
        // the author to decide its gate behavior rather than silently falling through.
