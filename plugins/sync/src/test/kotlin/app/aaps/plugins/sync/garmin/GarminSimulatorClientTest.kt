...
class GarminSimulatorClientTest: TestBase() {
    ...
    fun receiveMessage() {
        ...
        verify(receiver, timeout(5_000)).onConnect(client)
    ...
    fun sendMessage() {
        ...
        verify(receiver, timeout(5_000)).onSendMessage(eq(client), eq(device.id), eq(app.id), isNull())
    }
}