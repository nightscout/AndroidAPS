...
internal class OHAppIDDelegate @Inject internal constructor(
    ...
    operator fun getValue(thisRef: Any?, property: KProperty<*>): UUID {
        if (value == null) {
            val saved = preferences.getIfExists(OpenHumansUploaderPlugin.OhStringKey.AppId)
            if (saved.isNullOrBlank()) {
                val generated = UUID.randomUUID()
                value = generated
                preferences.put(OpenHumansUploaderPlugin.OhStringKey.AppId, generated.toString())
            } else {
