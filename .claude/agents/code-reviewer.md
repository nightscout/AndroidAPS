...
- **Compose ViewModel**: Must extend `ViewModel()`, use `@Inject constructor`, and register with `@ContributesIntoMap(AppScope::class, binding = binding<ViewModel>())` + `@ViewModelKey`. **Always use the graph** — never create manual `ViewModelProvider.Factory` classes. Shared helpers live in `core/ui/compose/ViewModelHelpers.kt` — all modules use this, never create per-module copies. In Compose use `metroViewModel()`.
