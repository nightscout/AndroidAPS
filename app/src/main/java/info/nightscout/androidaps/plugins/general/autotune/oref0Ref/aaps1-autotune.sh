...
if [ -z "$DIR" ]; then
    echo "Usage: aaps-autotune <--dir=myopenaps_directory> [--start-days-ago=number_of_days] [--end-days-ago=number_of_days] [--start-date=YYYY-MM-DD] [--end-date=YYYY-MM-DD] [--xlsx=autotune.xlsx] [--log=(true)|false] [--categorize-uam-as-basal=true|(false)] [--tune-insulin-curve=true|(false) ]"
exit 1
fi
...
echo "Grabbing AAPS treatments.json and entries/sgv.json for date range..."
...
do 
    ...
    echo "oref0-autotune-core aaps-autotune.$i.json profile.json profile.pump.json > newprofile.$i.json"
    if ! oref0-autotune-core aaps-autotune.$i.json profile.json profile.pump.json > newprofile.$i.json; then
        if cat profile.json | jq --exit-status .carb_ratio==null; then
            ...
            die "Could not run oref0-autotune-core aaps-autotune.$i.json profile.json profile.pump.json"
        fi
    else
