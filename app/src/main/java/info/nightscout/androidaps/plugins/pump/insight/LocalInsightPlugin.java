...
public class LocalInsightPlugin extends PluginBase implements PumpInterface, ConstraintsInterface, InsightConnectionService.StateCallback {
    ...
    public static LocalInsightPlugin getPlugin() {
        if (instance == null)
            instance = new LocalInsightPlugin();
        return instance;
    }


    public LocalInsightPlugin() {
        super(new PluginDescription().pluginName(R.string.insight_local).shortName(R.string.insightpump_shortname)
            .mainType(PluginType.PUMP).description(R.string.description_pump_insight_local)
            .fragmentClass(LocalInsightFragment.class.getName()).preferencesId(R.xml.pref_insight_local));
    ...
    protected void onStart() {
        super.onStart();
        MainApp.instance().bindService(new Intent(MainApp.instance(), InsightConnectionService.class),
            serviceConnection, Context.BIND_AUTO_CREATE);
        MainApp.instance().bindService(new Intent(MainApp.instance(), InsightAlertService.class), serviceConnection,
    ...
    public boolean isConnecting() {
        ...
        return state == InsightState.CONNECTING || state == InsightState.APP_CONNECT_MESSAGE
    ...
    public void getPumpStatus() {
        try {
            tbrOverNotificationBlock = ParameterBlockUtil.readParameterBlock(connectionService, Service.CONFIGURATION,
                TBROverNotificationBlock.class);
            readHistory();
            fetchBasalProfile();
            fetchLimitations();
    ...
    private void updatePumpTimeIfNeeded() throws Exception {
        ...
        if (Math.abs(calendar.getTimeInMillis() - System.currentTimeMillis()) > 10000) {
            ...
            Notification notification = new Notification(Notification.INSIGHT_DATE_TIME_UPDATED,
                MainApp.gs(R.string.pump_time_updated), Notification.INFO, 60);
    ...
    private void fetchBasalProfile() throws Exception {
        profileBlocks = ParameterBlockUtil.readParameterBlock(connectionService, Service.CONFIGURATION,
            BRProfile1Block.class).getProfileBlocks();
    }


    private void fetchStatus() throws Exception {
        if (statusLoaded) {
            GetPumpStatusRegisterMessage registerMessage = connectionService.requestMessage(
                new GetPumpStatusRegisterMessage()).await();
            ...
            if (registerMessage.isOperatingModeChanged())
                operatingMode = connectionService.requestMessage(new GetOperatingModeMessage()).await()
                    .getOperatingMode();
            if (registerMessage.isBatteryStatusChanged())
                batteryStatus = connectionService.requestMessage(new GetBatteryStatusMessage()).await()
                    .getBatteryStatus();
            if (registerMessage.isCartridgeStatusChanged())
                cartridgeStatus = connectionService.requestMessage(new GetCartridgeStatusMessage()).await()
                    .getCartridgeStatus();
            ...
            if (operatingMode == OperatingMode.STARTED) {
                if (registerMessage.isActiveBasalRateChanged())
                    activeBasalRate = connectionService.requestMessage(new GetActiveBasalRateMessage()).await()
                        .getActiveBasalRate();
                ...
                if (registerMessage.isActiveBolusesChanged())
                    activeBoluses = connectionService.requestMessage(new GetActiveBolusesMessage()).await()
                        .getActiveBoluses();
            } else {
    ...
    private void fetchLimitations() throws Exception {
        maximumBolusAmount = ParameterBlockUtil.readParameterBlock(connectionService, Service.CONFIGURATION,
            MaxBolusAmountBlock.class).getAmountLimitation();
        maximumBasalAmount = ParameterBlockUtil.readParameterBlock(connectionService, Service.CONFIGURATION,
            MaxBasalAmountBlock.class).getAmountLimitation();
        minimumBolusAmount = ParameterBlockUtil.readParameterBlock(connectionService, Service.CONFIGURATION,
            FactoryMinBolusAmountBlock.class).getAmountLimitation();
        minimumBasalAmount = ParameterBlockUtil.readParameterBlock(connectionService, Service.CONFIGURATION,
            FactoryMinBolusAmountBlock.class).getAmountLimitation();
    ...
    public PumpEnactResult setNewBasalProfile(Profile profile) {
        ...
        for (int i = 0; i < profile.getBasalValues().length; i++) {
            ...
            profileBlock
                .setDuration((((nextValue != null ? nextValue.timeAsSeconds : 24 * 60 * 60) - basalValue.timeAsSeconds) / 60));
    ...
    public double getBaseBasalRate() {
        ...
        if (activeBasalRate != null)
            return activeBasalRate.getActiveBasalRate();
        else
            return 0;
    }
        ...
        if (detailedBolusInfo.insulin > 0) {
            try {
                ...
                while (true) {
                    ...
                    OperatingMode operatingMode = connectionService.requestMessage(new GetOperatingModeMessage())
                        .await().getOperatingMode();
                    if (operatingMode != OperatingMode.STARTED)
                        break;
                    List<ActiveBolus> activeBoluses = connectionService.requestMessage(new GetActiveBolusesMessage())
                        .await().getActiveBoluses();
                    ...
                    if (activeBolus != null) {
                        ...
                        bolusingEvent.percent = (int)(100D / activeBolus.getInitialAmount() * (activeBolus
                            .getInitialAmount() - activeBolus.getRemainingAmount()));
                        bolusingEvent.status = MainApp.gs(R.string.insight_delivered, activeBolus.getInitialAmount()
                            - activeBolus.getRemainingAmount(), activeBolus.getInitialAmount());
                        ...
                        synchronized ($bolusLock) {
                            if (bolusCancelled || trials == -1 || trials++ >= 5) {
                                if (!bolusCancelled) {
                                    bolusingEvent.status = MainApp.gs(R.string.insight_delivered,
    ...
    public PumpEnactResult setTempBasalAbsolute(Double absoluteRate, Integer durationInMinutes, Profile profile,
        ...
        if (activeBasalRate == null)
            return result;
        if (activeBasalRate.getActiveBasalRate() == 0)
            return result;
    ...
    public PumpEnactResult cancelTempBasal(boolean enforceNew) {
        ...
        if (isFakingTempsByExtendedBoluses())
            cancelEBResult = cancelExtendedBolusOnly();
        PumpEnactResult cancelTBRResult = cancelTempBasalOnly();
    ...
    private PumpEnactResult cancelExtendedBolusOnly() {
        ...
        try {
            for (ActiveBolus activeBolus : activeBoluses) {
                if (activeBolus.getBolusType() == BolusType.EXTENDED
                    || activeBolus.getBolusType() == BolusType.MULTIWAVE) {
                    alertService.ignore(AlertType.WARNING_38);
                    CancelBolusMessage cancelBolusMessage = new CancelBolusMessage();
                    ...
                    confirmAlert(AlertType.WARNING_38);
                    alertService.ignore(null);
                    InsightBolusID insightBolusID = MainApp.getDbHelper().getInsightBolusID(
                        connectionService.getPumpSystemIdentification().getSerialNumber(), activeBolus.getBolusID(),
    ...
    private void confirmAlert(AlertType alertType) {
        try {
            ...
            while (System.currentTimeMillis() - started < 10000) {
                GetActiveAlertMessage activeAlertMessage = connectionService
                    .requestMessage(new GetActiveAlertMessage()).await();
    ...
    public PumpEnactResult setTBROverNotification(boolean enabled) {
        ...
        try {
            ...
            log.info("Exception while updating TBR notification block: " + e.getClass().getCanonicalName() + " ("
                + e.getErrorCode() + ")");
            result.comment = ExceptionTranslator.getString(e);
        } catch (InsightException e) {
            tbrOverNotificationBlock.setEnabled(valueBefore);
    ...
    public String shortStatus(boolean veryShort) {
        ...
        if (activeTBR != null) {
            ret.append(MainApp.gs(R.string.short_status_tbr, activeTBR.getPercentage(), activeTBR.getInitialDuration()
                - activeTBR.getRemainingDuration(), activeTBR.getInitialDuration())
                + "\n");
        }
        if (activeBoluses != null)
            for (ActiveBolus activeBolus : activeBoluses) {
                if (activeBolus.getBolusType() == BolusType.STANDARD)
                    continue;
                ret.append(MainApp.gs(
                    activeBolus.getBolusType() == BolusType.MULTIWAVE ? R.string.short_status_multiwave
                        : R.string.short_status_extended, activeBolus.getRemainingAmount(), activeBolus
                        .getInitialAmount(), activeBolus.getRemainingDuration())
                    + "\n");
            }
    ...
    private void readHistory() {
        try {
            ...
            timeOffset = Calendar.getInstance(TimeZone.getTimeZone("UTC")).getTimeInMillis()
                - parseDate(pumpTime.getYear(), pumpTime.getMonth(), pumpTime.getDay(), pumpTime.getHour(),
                    pumpTime.getMinute(), pumpTime.getSecond());
            ...
            try {
                ...
                if (historyOffset == null) {
                    ...
                    historyEvents = connectionService.requestMessage(new ReadHistoryEventsMessage()).await()
                        .getHistoryEvents();
                } else {
                    StartReadingHistoryMessage startMessage = new StartReadingHistoryMessage();
                    ...
                    connectionService.requestMessage(startMessage).await();
                    while (true) {
                        List<HistoryEvent> newEvents = connectionService.requestMessage(new ReadHistoryEventsMessage())
                ...
                log.info("Exception while reading history: " + e.getClass().getCanonicalName() + " ("
                    + e.getErrorCode() + ")");
            } catch (InsightException e) {
        ...
        new Handler(Looper.getMainLooper()).post(() -> MainApp.bus().post(
            new EventRefreshOverview("LocalInsightPlugin::readHistory")));
    }
    ...
    private void processHistoryEvents(String serial, List<HistoryEvent> historyEvents) {
        ...
        for (InsightPumpID pumpID : pumpStartedEvents) {
            ...
            if (stoppedEvent == null || stoppedEvent.eventType.equals("PumpPaused"))
                continue;
    ...
    private boolean processHistoryEvent(String serial, List<TemporaryBasal> temporaryBasals,
            List<InsightPumpID> pumpStartedEvents, HistoryEvent event) {
        if (event instanceof DefaultDateTimeSetEvent)
            return false;
            ...
            processTubeFilledEvent((TubeFilledEvent)event);
        else if (event instanceof SniffingDoneEvent)
            processSniffingDoneEvent((SniffingDoneEvent)event);
        else if (event instanceof PowerUpEvent)
            processPowerUpEvent((PowerUpEvent)event);
    ...
    private void processDateTimeChangedEvent(DateTimeChangedEvent event) {
        ...
        long timeBefore = parseDate(event.getBeforeYear(), event.getBeforeMonth(), event.getBeforeDay(),
            event.getBeforeHour(), event.getBeforeMinute(), event.getBeforeSecond());
    ...
    private void processTubeFilledEvent(TubeFilledEvent event) {
        if (!SP.getBoolean("insight_log_tube_changes", false))
            return;
        long timestamp = parseDate(event.getEventYear(), event.getEventMonth(), event.getEventDay(),
            event.getEventHour(), event.getEventMinute(), event.getEventSecond())
            + timeOffset;
    ...
    private void processSniffingDoneEvent(SniffingDoneEvent event) {
        if (!SP.getBoolean("insight_log_site_changes", false))
            return;
        long timestamp = parseDate(event.getEventYear(), event.getEventMonth(), event.getEventDay(),
            event.getEventHour(), event.getEventMinute(), event.getEventSecond())
            + timeOffset;
    ...
    private void processPowerUpEvent(PowerUpEvent event) {
        if (!SP.getBoolean("insight_log_battery_changes", false))
            return;
        long timestamp = parseDate(event.getEventYear(), event.getEventMonth(), event.getEventDay(),
            event.getEventHour(), event.getEventMinute(), event.getEventSecond())
            + timeOffset;
    ...
    private void processOperatingModeChangedEvent(String serial, List<InsightPumpID> pumpStartedEvents,
            OperatingModeChangedEvent event) {
        long timestamp = parseDate(event.getEventYear(), event.getEventMonth(), event.getEventDay(),
            event.getEventHour(), event.getEventMinute(), event.getEventSecond())
            + timeOffset;
    ...
    private void processBolusDeliveredEvent(String serial, BolusDeliveredEvent event) {
        long timestamp = parseDate(event.getEventYear(), event.getEventMonth(), event.getEventDay(),
            event.getEventHour(), event.getEventMinute(), event.getEventSecond())
            + timeOffset;
        long startTimestamp = parseRelativeDate(event.getEventYear(), event.getEventMonth(), event.getEventDay(),
            event.getEventHour(), event.getEventMinute(), event.getEventSecond(), event.getStartHour(),
            event.getStartMinute(), event.getStartSecond())
            + timeOffset;
        InsightBolusID bolusID = MainApp.getDbHelper().getInsightBolusID(serial, event.getBolusID(), timestamp);
        if (bolusID == null || bolusID.endID != null) {
            bolusID = new InsightBolusID();
    ...
    private void processOccurrenceOfAlertEvent(OccurrenceOfAlertEvent event) {
        if (!SP.getBoolean("insight_log_alerts", false))
            return;
        long timestamp = parseDate(event.getEventYear(), event.getEventMonth(), event.getEventDay(),
            event.getEventHour(), event.getEventMinute(), event.getEventSecond())
            + timeOffset;
    ...
    private long parseRelativeDate(int year, int month, int day, int hour, int minute, int second, int relativeHour,
            int relativeMinute, int relativeSecond) {
    ...
    public Constraint<Double> applyBolusConstraints(Constraint<Double> insulin) {
        if (!limitsFetched)
            return insulin;
        insulin
            .setIfSmaller(maximumBolusAmount,
                String.format(MainApp.gs(R.string.limitingbolus), maximumBolusAmount, MainApp.gs(R.string.pumplimit)),
                this);
        if (insulin.value() < minimumBolusAmount) {
            ...
            insulin.set(0d,
                String.format(MainApp.gs(R.string.limitingbolus), minimumBolusAmount, MainApp.gs(R.string.pumplimit)),
                this);
        }
        return insulin;
    }
    ...
    public void onStateChanged(InsightState state) {
        if (state == InsightState.CONNECTED) {
            statusLoaded = false;
            new Handler(Looper.getMainLooper()).post(() -> MainApp.bus().post(
                new EventDismissNotification(Notification.INSIGHT_TIMEOUT_DURING_HANDSHAKE)));
            ...
            new Handler(Looper.getMainLooper()).post(() -> MainApp.bus().post(
                new EventRefreshOverview("LocalInsightPlugin::onStateChanged")));
        }
