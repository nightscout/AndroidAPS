 ...
 * because `:pump:dana:danar:emulator` sits on `:pump:dana:common` rather than on the driver, so the
 * driver is free to depend on the emulator without a cycle. The app now names no Dana R type at all.
 */
