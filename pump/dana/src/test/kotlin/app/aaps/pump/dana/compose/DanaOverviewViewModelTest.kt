...
/**
 * Unit test for the SHARED Dana overview state logic. [DanaOverviewViewModel] is the open base class
 * that both DanaR and DanaRS drive (DanaRSOverviewViewModel extends it), so this covers the
 * PumpOverviewUiState assembly for both variants. The rendering itself is covered by core:ui's
 * PumpOverviewScreenTest (DanaOverviewScreen just delegates to PumpOverviewScreen).
 */
@OptIn(ExperimentalCoroutinesApi::class)
internal class DanaOverviewViewModelTest {
    ...
    fun setUp() {
        ...
        // remaining info-row labels rendered in the configured state (unstubbed -> null label -> NPE)
    ...
    fun configured_buildsInfoRows_andBatteryWarnLevel() {
        ...
        whenever(danaPump.serialNumber).thenReturn("")            // empty -> serial row skipped, avoids NPE on isNotEmpty
