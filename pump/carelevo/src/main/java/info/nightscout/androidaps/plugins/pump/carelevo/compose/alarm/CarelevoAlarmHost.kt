package info.nightscout.androidaps.plugins.pump.carelevo.compose.alarm
...
import info.nightscout.androidaps.plugins.pump.carelevo.ext.transformStringResources
import info.nightscout.androidaps.plugins.pump.carelevo.presentation.model.AlarmEvent
import info.nightscout.androidaps.plugins.pump.carelevo.presentation.viewmodel.CarelevoAlarmViewModel
...
private fun buildDescArgsFor(context: android.content.Context, alarm: CarelevoAlarmInfo): List<String> = when (alarm.cause) {
    ...
    AlarmCause.ALARM_NOTICE_BG_CHECK -> {
        ...
        listOf(
            when {
                hours > 0 && minutes > 0 ->
                    context.getString(info.nightscout.androidaps.plugins.pump.carelevo.R.string.common_label_unit_value_duration_hour_and_minute, hours, minutes)
                hours > 0 ->
                    context.getString(info.nightscout.androidaps.plugins.pump.carelevo.R.string.common_label_unit_value_duration_hour, hours)
                else ->
                    context.getString(info.nightscout.androidaps.plugins.pump.carelevo.R.string.common_label_unit_value_minute, minutes)
            }
        )
    }
