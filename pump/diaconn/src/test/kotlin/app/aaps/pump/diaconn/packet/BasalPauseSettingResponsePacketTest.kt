...
class BasalPauseSettingResponsePacketTest : TestBaseWithProfile() {

    private lateinit var diaconnG8Pump: DiaconnG8Pump

    private val packetInjector = HasAndroidInjector {
        AndroidInjector {
            if (it is BasalPauseSettingResponsePacket) {
    ...
    fun handleMessageShouldParseOtpNumber() {
        // Given
        val packet = BasalPauseSettingResponsePacket(packetInjector)
    ...
    fun msgTypeShouldBeCorrect() {
        ...
        // Then
        assertThat(packet.msgType).isEqualTo(0x83.toByte())
    }
    ...
    fun friendlyNameShouldBeCorrect() {
        ...
        // Then
        assertThat(packet.friendlyName).isEqualTo("PUMP_BASAL_PAUSE_SETTING_RESPONSE")
    }

    private fun createValidPacket(otpNumber: Int): ByteArray {
        val data = ByteArray(20)
        data[0] = 0xef.toByte() // SOP
        data[1] = 0x83.toByte() // msgType
