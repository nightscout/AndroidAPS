...
class InjectionMealSettingResponsePacketTest : TestBaseWithProfile() {

    private lateinit var diaconnG8Pump: DiaconnG8Pump

    private val packetInjector = HasAndroidInjector {
        AndroidInjector {
            if (it is InjectionMealSettingResponsePacket) {
    ...
    fun handleMessageShouldParseOtpNumber() {
        // Given
        val packet = InjectionMealSettingResponsePacket(packetInjector)
    ...
    fun msgTypeShouldBeCorrect() {
        ...
        // Then
        assertThat(packet.msgType).isEqualTo(0x86.toByte())
    }
    ...
    fun friendlyNameShouldBeCorrect() {
        ...
        // Then
        assertThat(packet.friendlyName).isEqualTo("PUMP_INJECTION_MEAL_SETTING_RESPONSE")
    }

    private fun createValidPacket(otpNumber: Int): ByteArray {
        val data = ByteArray(20)
        data[0] = 0xef.toByte() // SOP
        data[1] = 0x86.toByte() // msgType
