...
class DisplayTimeoutSettingResponsePacketTest : TestBaseWithProfile() {

    private lateinit var diaconnG8Pump: DiaconnG8Pump

    private val packetInjector = HasAndroidInjector {
        AndroidInjector {
            if (it is DisplayTimeoutSettingResponsePacket) {
    ...
    fun handleMessageShouldParseOtpNumber() {
        // Given
        val packet = DisplayTimeoutSettingResponsePacket(packetInjector)
    ...
    fun handleMessageShouldFailOnInvalidResult() {
        // Given
        val packet = DisplayTimeoutSettingResponsePacket(packetInjector)
        val data = createPacketWithResult(1) // Invalid result for setting response
    ...
    fun friendlyNameShouldBeCorrect() {
        ...
        // Then
        assertThat(packet.friendlyName).isEqualTo("PUMP_DISPLAY_TIMEOUT_SETTING_RESPONSE")
    }
