...
class InjectionCancelSettingResponsePacketTest : TestBaseWithProfile() {

    private lateinit var diaconnG8Pump: DiaconnG8Pump

    private val packetInjector = HasAndroidInjector {
        AndroidInjector {
            if (it is InjectionCancelSettingResponsePacket) {
    ...
    fun handleMessageShouldParseOtpNumber() {
        // Given
        val packet = InjectionCancelSettingResponsePacket(packetInjector)
    ...
    fun msgTypeShouldBeCorrect() {
        ...
        // Then
        assertThat(packet.msgType).isEqualTo(0xAB.toByte())
    }
    ...
    fun friendlyNameShouldBeCorrect() {
        ...
        // Then
        assertThat(packet.friendlyName).isEqualTo("PUMP_INJECTION_CANCEL_SETTING_RESPONSE")
    }

    private fun createValidPacket(otpNumber: Int): ByteArray {
        val data = ByteArray(20)
        data[0] = 0xef.toByte() // SOP
        data[1] = 0xAB.toByte() // msgType
