    ...
    */
    ...
    private fun connect(connectionWaitCond: ConnectionWaitCondition): Observable<PodEvent> = Observable
        .create { emitter ->
            ...
            try {
                ...
                aapsLogger.debug(LTag.PUMPBTCOMM, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX Bond state ${podDevice.bondState}")
                if (podDevice.bondState == BluetoothDevice.BOND_NONE) {
                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S || ActivityCompat.checkSelfPermission(context, Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED) {
                        val result = podDevice.createBond()
                        aapsLogger.debug(LTag.PUMPBTCOMM, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX Bonding with pod resulted $result")
                        Thread.sleep(10000)
                        aapsLogger.debug(LTag.PUMPBTCOMM, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX Bond state #2 ${podDevice.bondState}")
                    }
                }
    ...
    override fun removeBond() {
        try {
            ...
            aapsLogger.debug(LTag.PUMPBTCOMM, "XXXXXX Remove bond resulted $result")
        } catch (t: Throwable) {
