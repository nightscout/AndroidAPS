...
public class LocalInsightPlugin extends PumpPluginBase implements Pump, Constraints, InsightConnectionService.StateCallback {
    ...
    public PumpEnactResult deliverTreatment(DetailedBolusInfo detailedBolusInfo) {
        ...
        if (insulin > 0) {
            try {
                ...
                bolusingEvent.setStatus(rh.gs(R.string.insight_delivered, 0d, insulin * concentration));
                ...
                while (true) {
                    ...
                    if (activeBolus != null) {
                        ...
                        bolusingEvent.setStatus(rh.gs(R.string.insight_delivered, (activeBolus.getInitialAmount() - activeBolus.getRemainingAmount())*concentration, activeBolus.getInitialAmount() * concentration));
                        ...
                        synchronized ($bolusLock) {
                            if (bolusCancelled || trials == -1 || trials++ >= 5) {
                                if (!bolusCancelled) {
                                    bolusingEvent.setStatus(rh.gs(R.string.insight_delivered, insulin*concentration, insulin*concentration));
    ...
    public String shortStatus(boolean veryShort) {
        ...
        if (activeBoluses != null) for (ActiveBolus activeBolus : activeBoluses) {
            ...
            ret.append(rh.gs(activeBolus.getBolusType() == BolusType.MULTIWAVE ?
                            R.string.short_status_multiwave : R.string.short_status_extended,
                    activeBolus.getRemainingAmount() * concentration,
                    activeBolus.getInitialAmount() * concentration,
                    activeBolus.getRemainingDuration())).append("\n");
        }
        if (!veryShort && totalDailyDose != null) {
            ret.append(rh.gs(R.string.short_status_tdd, totalDailyDose.getBolusAndBasal()*concentration)).append("\n");
        }
        if (cartridgeStatus != null) {
            ret.append(rh.gs(R.string.short_status_reservoir, cartridgeStatus.getRemainingAmount() * concentration)).append("\n");
        }
