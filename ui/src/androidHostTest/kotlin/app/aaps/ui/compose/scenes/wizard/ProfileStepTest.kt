...
class ProfileStepTest {
    ...
    @Before
    fun setUp() {
        title = RuntimeEnvironment.getApplication().getString(R.string.careportal_profileswitch)
    }
    ...
    fun rendersTitle_andToggleReflectsDisabledState() {
        compose.setContent {
            MaterialTheme {
                ProfileStep(
                    state = previewState.copy(profileEnabled = false),
                    onToggle = {}, onUpdate = {},
                    profileNames = listOf("Default", "Sport"),
                    onBack = {}, onNext = {}
                )
            }
        }
