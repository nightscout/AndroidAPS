...
class TreatmentViewModel @Inject constructor(
    ...
    fun refreshState() {
        viewModelScope.launch {
            ...
            // Check if CGM/Calibration buttons should be available
            val showCgmButton = xDripSource.isEnabled() || dexcomBoyda.isEnabled()
            val showCalibrationButton = xDripSource.isEnabled() && iobCobCalculator.ads.actualBg() != null

            // Check preferences for button visibility
            val showCgm = showCgmButton && preferences.get(BooleanKey.OverviewShowCgmButton)
            val showCalibration = showCalibrationButton && preferences.get(BooleanKey.OverviewShowCalibrationButton)
