 ...
 *
 * The engine is **OkHttp**, so this reuses the HTTP stack the app already ships rather than adding a
 * second one. When the module builds for iOS the engine becomes Darwin and nothing else here
 * changes - that is the reason for moving off Retrofit at all.
 *
...
internal object NsKtorClient {
    ...
    fun build(logging: Boolean, logger: (String) -> Unit): HttpClient = HttpClient(OkHttp) {
        expectSuccess = false
