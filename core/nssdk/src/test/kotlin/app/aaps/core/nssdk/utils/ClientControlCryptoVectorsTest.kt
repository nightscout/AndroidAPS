 ...
 *
 * These are fixed inputs with their exact expected outputs, produced by the current JVM
 ...
 * HMAC-SHA256".
 *
 * That distinction matters because the wire format is already frozen: AAPS masters and clients in
 * the field exchange these values today. The primitives themselves are standards and interoperate by
 ...
 * - **PIN encoding.** `PBEKeySpec` takes a char array; the PIN is ASCII digits, so any sane encoding
 *   agrees - but a non-ASCII PIN would not, which is worth knowing before anyone widens the alphabet.
 *
...
class ClientControlCryptoVectorsTest {
    ...
    fun `hmac vector - empty string`() {
        assertThat(ClientControlCrypto.sign(secret, "")).isEqualTo("d38b42096d80f45f826b44a9d5607de72496a415d3f4a1a8c88e3bb9da8dc1cb")
    }

    /** Non-ASCII input, so the UTF-8 encoding of the signed text is pinned too. */
    @Test
    fun `hmac vector - non ascii`() {
        assertThat(ClientControlCrypto.sign(secret, "poznámka ěščřž")).isEqualTo("fcefc8439c7d9f1d149bee6a4dfa2ec4631f8d23a8fd35bc9444c86a4a19b36d")
    }
    ...
    fun `signatures are lower case hex of 64 characters`() {
        ...
        assertThat(signature).isEqualTo(signature.lowercase())
        assertThat(signature.all { it.isDigit() || it in 'a'..'f' }).isTrue()
    }
    ...
    fun `wrap vector - pairing payload`() {
        val plaintext = """{"clientId":"abc","secret":"00","expiresAt":1785992179555}""".toByteArray(Charsets.UTF_8)

        val wrapped = ClientControlPairingCrypto.wrap(plaintext, pin = "12345678", salt = salt, iv = iv)

        assertThat(hex(wrapped)).isEqualTo("f864c764382a5e82af17fccf545a14fea3b29eae0784eb2b1630ffe179e58b31c5fc8291afde74f7a0867579b244c6213a12eb9bf1382b1b6c569d2d1f74c25b52c6b253943d87026018")
    }
    ...
    fun `wrap vector - short plaintext`() {
        ...
        assertThat(hex(wrapped)).isEqualTo("f0ef78af68d41f05d7bfed02b15a252856f3620c")
    }
     ...
     */
    @Test
    fun `the gcm tag is appended, making the output 16 bytes longer`() {
        val plaintext = ByteArray(40) { it.toByte() }

        val wrapped = ClientControlPairingCrypto.wrap(plaintext, pin = "12345678", salt = salt, iv = iv)

        assertThat(wrapped.size).isEqualTo(plaintext.size + 16)
    }
    ...
    fun `wrap and unwrap round trip`() {
        val plaintext = "hello pairing".toByteArray(Charsets.UTF_8)
    ...
    fun `a wrong pin gives null`() {
        val wrapped = ClientControlPairingCrypto.wrap("x".toByteArray(), pin = "11111111", salt = salt, iv = iv)

        assertThat(ClientControlPairingCrypto.unwrap(wrapped, pin = "22222222", salt = salt, iv = iv)).isNull()
    }
    ...
    fun `a tampered blob gives null`() {
        ...
        assertThat(ClientControlPairingCrypto.unwrap(wrapped, pin = "11111111", salt = salt, iv = iv)).isNull()
    }
}
