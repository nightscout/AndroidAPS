...
class PrepareGraphDataRunner @Inject constructor(
    ...
    suspend fun run(job: String?, generation: Long, isStopped: () -> Boolean): WorkOutcome {
        ...
        // True once this run no longer owns the chain: it was stopped, or a newer generation
        // replaced it. Asked right before the result is published, see publishAds().
     ...
     * this run still owns the chain. A cancelled run keeps going until its next
     * isStopped check (up to one bucket, tens of seconds on slow devices), while
     * stopCalculation() can return before that. Without this check the old run writes
     * its stale clone over the store that the replacing chain has already invalidated,
     * a lost update (issue #5066).
