...
class CommandQueueImplementation @Inject constructor(
     ...
     *    "wrong basal until fixed" card. By default it rings via [AlarmSound.BOLUS_ERROR], but the user can
    ...
    internal fun postProfileWriteResult(result: PumpEnactResult?, silent: Boolean): Boolean {
        if (result == null || !result.success) {
            val sound = if (preferences.get(BooleanKey.AlertFailedUpdateBasalProfileSound)) AlarmSound.BOLUS_ERROR else null
