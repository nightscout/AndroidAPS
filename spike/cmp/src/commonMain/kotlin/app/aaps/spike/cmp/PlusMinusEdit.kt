...
/**
 * Copied VERBATIM from `core/ui/.../compose/PlusMinusEdit.kt`, changing only the package.
 *
 * Chosen on purpose as the hardest realistic case rather than the easiest: `TextField` plus
 * `KeyboardOptions`, `KeyboardActions`, `ImeAction`, focus handling and text selection is exactly the
 * area where Compose Multiplatform on iOS is weakest, so a spike that only draws cards and icons
 * would prove nothing. It also pulls in `NumberFormat` from `:core:data` and `TextRef` from
 * `:core:keys` - both already multiplatform - so this compiles the UI on top of the real shared spine
 * rather than against stubs.
 *
